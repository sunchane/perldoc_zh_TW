...
Catalyst::Manual::Components - Catalyst 應用程式可重用的元件
...
本章節列出一些可以讓 Catalyst 執行期間延長功能性的元件 (或是外掛)。
大部分的元件沒有包含在 Catalyst 發行版內但是可以在 CPAN 下載。
同時也會需要安裝 CPAN 額外的依賴模組。
...
這些列表會隨時間的關係過時，有些外掛甚至會棄用，或相反地，也有可以以後會收錄在 L<Catalyst> 核心模組。
請確認您的 Catalyst:: 及 CatalystX:: 模組名稱空間查看額外的模元件，
並且到新聞群組請求協助與建議 ( L<http://wiki.catalystframework.org/wiki/Support> ) 
套件的開發狀態還是您的外掛/框架如何選用。
...
請謹慎使用您的外掛！通常您要問 "這個外掛應該放在我的應用程式全域範圍嗎？"。
如果不是那它應該成為控制器角色 (Controller Role) (範例請看 L<Catalyst::TraitFor::Controller::reCAPTCHA>。

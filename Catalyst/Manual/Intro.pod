...
C<:Regex> 批配全域的動作，例如不包含參照到它們呼叫的名稱空間。
所以前面的例子B<不會>批配成功 C<http://localhost:3000/my/controller/item23/order42> 這個 URL -
除非請使用 C<:LocalRegex> 動作。
...
C<:LocalRegex> 動作運用在區域範圍，舉例來說會先批配名稱空間路徑。
上面的範例會匹配 URL 像是 
C<http://localhost:3000/my/controller/widget23>.
...
跟之前範例不同的是會匹配成功 C<http://localhost:3000/my/controller/foo/widget23> 
- 也可能會比對到其他的路徑。
...
對於 C<:LocalRegex> 及 C<:Regex> 這兩個動作，如果您使用正規表達式小括號來分離比對的 URL 樣本，
這些變數會放在 C<< $c->req->captures >> 陣列。
上面的範例會把 "widget23" 分離擷取成 "23"，並且 C<< $c->req->captures->[0] >> 的值為 "23"。
如果您想要傳遞參數比對 URL 的末端，您必須使用正規表達式的樣本鍵達成。
請看下面章節 L</URL Path Handling>。
...
會比對 C</catalog/*/item/*> 路徑。
匹配成功後會由上而下呼叫其他路徑的動作 - C<catalog()> 被呼叫時處理它的元件，
然後 C<item()> 被呼叫時也處理它元件。
更多有關分派類型，請看 L<Catalyst::DispatchType::Chained>。
...
私有不會匹配任何 URL - 只提供給 Catalyst 程式上私有的動作呼叫，
URL 發出請求時也不會自動被呼叫。
...
Catalyst C<:Private> 修飾器與其他修飾器區分並不能一起運作 (例如不能把這個修飾器與 C<:Path> 和 C<:Chained> 結合使用)。
...
Private 動作只能夠明確地在 Catalyst 應用程式內部使用。
您可能會在控制器內呼叫 Catalyst 方法透過 C<forward> 或 C<detach> 啟動它。
...
請看 L</Flow Control> 查閱完整有關傳遞請求到其他動作。要注意的是，
當您轉發其他元件的副程式時，您必須要使用完整路徑到這些方法，所以 C<MyApp::Controller::Catalog::Order::Process> 的私有方法 C<bar>，
在其他地方呼叫時要用 << $c->forward('/catalog/order/process/bar') >> 才能到達。
...
B<附記：>看過這些範例後，您大概會已經知道定義副程式名稱及路徑動作的重點了。
然而，每一個公開 (public) 的方法也包含私有的，並使用名稱空間及副程式名稱來回應B<路徑>，
所以統一的作法是使用 C<forward> 方法來定位這些動作。
...
如果存在，特殊動作 C< index >, C< auto >, C<begin>, C<end> 及 C< default > 
會在請求期間於特定的位置被呼叫。
...
為了回應特定的應用程式狀態 (states)，Catalyst 會自動呼叫這些應用程式類別中的內建動作：
...
如果沒有其他動作匹配時會被呼叫。
可以應用在舉例來說，顯示主要應用程式的首頁，或是每一個控制器的錯誤頁。
B<請注意>：Catalyst 為了看待 C<default : Private> 粗略來說是同等的。
...
C<index> 跟 C<default> 非常相似除了不使用引數外，匹配的比重也稍微高了一點。
對於靜態的控制器入口很有用，例如靜態的歡迎畫面。
請注意它也比 C<Path> 比重高出一些。
實際上 C<index> 副程式可以呼較任何您想要的。
B<請注意>：為了 Catalyst 應用程式，您會看到 C<index : Private> 被使用，粗略來說同等。
...
請求的一開始呼叫，控制器將要運行時被識別，但是在 URL 匹配動作呼叫之前。
Catalyst 會呼叫控制器中的 C<begin> 函式，控制器內也是 URL 比對動作放置的地方。
...
請求結束時被呼叫，在所有的 URL 比對動作呼叫之後。
Catalyst 會呼叫控制器中的 C<end> 函式，其中包含各種 URL 比對動作。
...
除了一般的內建動作，你也可以有特殊的動作提供給鏈接，C<auto>。
C<auto> 動作會在 C<begin> 之後，在您的 URL 比對動作之前運行。
不像其他的內建動作，可以呼叫B<多重的> C<auto> 動作；
並且會依序被呼叫，隨著應用程式及透過指定的類別啟用。

=back

=head4 Built-in actions in controllers/autochaining

=head4 內建動作 controllers/autochaining
...
您可以在您的控制器定義內建動作就像是在應用程式類別中一樣。
換句話說，前面三個內建動作，只有一個會在請求循環時被運行。
所以如果 C<MyApp::Controller::Catalog::begin> 存在，名稱空間在 C<catalog> 中，會取代 C<MyApp::begin> 動作。
以此類推 C<MyApp::Controller::Catalog::Order::begin> 也會覆寫前面的動作。
...
但是 C<auto> 不會像這樣取代，假設下面這些存在，
C<MyApp::Controller::Root::auto>, C<MyApp::Controller::Catalog::auto> 及
C<MyApp::Catalog::Order::auto> 它們會依序執行。

Here are some examples of the order in which the various built-ins
would be called:

這裡有一些例子，各種內建動作會依序被呼叫：
...
C<auto> 動作事實上是被您破壞了程序鏈接變回 0 而有所不同。
如果 C<auto> 動作傳回傳 0，任何剩下的動作會被跳過，C<end> 例外。
所以，如前面的請求順序的例子，如果第一的 B<auto> 動作回傳 B<false>，
鏈接的結果會像這樣子：
...
=item 對於請求 C</foo/bar/foo> 第一個 C<auto> 回傳 false 的情況 
...
您也可以在 C<auto> 動作內使用 C<dir> 函式；這種情況，情求就會直接今入最後階段，而不執行進一步的動作。
所以之前的例子，C<MyApp::Controller::Foo::Bar::end> 也會被跳過。
...
為什麼要使用 C<auto> 這個例子為有關認證的動作：
您可以設置 C<auto> 動作處理您應用程式類別的認證工作 (總是會最先運行)，
如果這個 URL 出現的認證失敗，回傳 0 之後就跳過剩下的動作。

B<Note:> Looking at it another way, C<auto> actions have to return a
true value to continue processing!

B<請注意：> 如果用其他路徑訪問，C<auto> 動作可能會回傳 true 而繼續運行！
...
您可以用 URL 以斜線 (/) 區分作為引數傳遞給應用程式。
如果動作是以正規表達式 Regex 或 LocalRegex 傳遞，'$' 錨點 (anchor) 必須使用。
例如假設您要處理 C</foo/$bar/$baz>，C<$bar> 和 C<$baz> 

    sub foo : Regex('^foo$') { my ($self, $context, $bar, $baz) = @_; }

But what if you also defined actions for C</foo/boo> and C</foo/boo/hoo>?

但是假如您同時定義給 C</foo/boo> 和 C</foo/boo/hoo> 動作時？
...
Catalyst 會以最具體到最不具體的順序批配 - 也就是說，匹配路徑最多的部份獲勝：
...
所以 Catalyst 不會錯誤地分派前兩個 URL 給 '^foo$' 動作。
...
如果 Regex 或 LocalRegex 動作沒使用 '$' 錨點，動作依然會匹配 URL 包含的引數；
但是這些引數就無法透過 C<@_> 使用，因為 Regex 會吃掉它們。
...
請小心！ 如果您編寫兩種比對，匹配相同的路徑，相同的特徵 (同樣也比對相同的路徑長度)，
就不保證會呼叫到那一個。
最好編寫成沒有正規表達式的先嘗試，之後才比對正規表達式，但是如果您堅持使用請照下面例子：
...
以 URL I<(GET)> 請求做為參數的傳遞是由 L<Catalyst::Request> 類別在處理的。
C<param> 方法等同於 L<CGI.pm|CGI> 的方法，您可以在模型中使用它。
...
您可以使用 C<forward> 控制應用程式，可以執行要動作的鍵。
可以是同一個控制器或其他控制器中的動作，或類別名稱並使用可選的類別方法。
C<forward> 之後，控制流程會回傳 C<forward> 發出的方法。
...
C<forward> 和方法呼叫類似。主要的不同之處在於 C<forward> 使用包裹 (wraps) 以利處理例外；
它會自動傳遞語境物件 (C<$c> 或 C<$context>)；
並且允許剖析每一個呼叫 (如開啟除錯模式時會顯示日誌)。
...
C<Forward> 不會新增一個請求，所以您的請求物件 (C<< $c->req >>) 保持不變。
這是使用 C<forward> 和發起轉址關鍵不同之處。
...
您看到了這個範例，相同的控制器裡您可以直接使用方法名稱，指向控制器內定義好的動作方法。
如果您要轉發 (forward) 自其他的控制器，或主要應用程式，您必須指向完整的路徑名稱。
...
這個機制 L<Catalyst::Action::RenderView> 在視圖類別轉發C<處理的>方法。
...
您應該注意雖然 B<forward> 很有用，但是不是只有這個方法呼叫 Catalyst 其他的程式。
轉發只是給您的螢幕除錯資訊，包裝您呼叫的程式在本地異常處理中。

If you don't want or need these features then it's perfectly acceptable
(and faster) to do something like this:

如果您不需要使用這個功能，完美的方法 (也比較快) 可以是用像：
...
請注意轉發 C<forward> 呼叫動作完成後會繼續運行。如果您要讓呼叫動作停止後續的程序，
使用 C<detach> 代替，C<detach> 後的動作執行完之後不會回傳給呼叫方。
這兩種情況，Catalyst 會自動呼叫 process() 如果您忽略這個方式。
...
Catalyst 內建網頁伺服器可以測試本地部屬。
(之後，我們會使用更強大的伺服器，使用 Apache/mod_perl 或 FastCGI 作為範例，作為生產階段環境。)
...
...然後用瀏覽器造訪 http://localhost:3000/ 來觀看輸出。
...
Catalyst 有很多應用程式回歸測試的工具。
B<幫助腳本>自動產生簡單的測試，之後可以根據您的專案擴充這個腳本。
編寫您自己全面的測試腳本，請看 L<Test::WWW::Mechanize::Catalyst> 這個獨立的工具。

For more testing ideas, see L<Catalyst::Manual::Tutorial::08_Testing>.

更多測試的概念，請看 L<Catalyst::Manual::Tutorial::08_Testing>。

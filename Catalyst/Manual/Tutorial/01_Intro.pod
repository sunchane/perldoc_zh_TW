...
Catalyst::Manual::Tutorial::01_Intro - Catalyst 教學手冊 - Chapter 1: 簡介

=head1 OVERVIEW

This is B<Chapter 1 of 10> for the Catalyst tutorial.

這是教學手冊 B<Chapter 1 of 10>。
...
本教學手冊提供幾個章節介紹 Catalyst 網頁框架。
讓您快速導覽最常使用的幾個功能。
在幾乎所有 Catalyst 應用程序的構建中。
以及專注在世界真實被實踐及被需求的 Catalyst 應用程式。
...
雖然教學手冊主要的客群為 Catalyst 新手，有經驗的使用者可以瀏覽特定章節 
(例如，如何在他們的模型類別中使用 DBIC，或如何在既有的應用程式增加認證及授權，並且管理)。

The most recent code for the tutorial is included on the Tutorial Virtual
Machine you can download from:

教學手冊最新的原始檔放在虛擬電腦的映像檔中，您可以在這裡下載：
...
您應該會想要直接下載原始檔，用下面的指令抓取 (請注意未來可能會使用 GIT)：
...
這樣會下載教學手冊各章節最新的原始檔，放進 CatalystTutorial 這個資料夾在您的機器。
...
這本教學手冊參考的實做機器會提供給您，可以執行程式碼來確定作業系統配置正確
(如果您使用虛擬電腦映像檔應該不會有這種問題，:-) ) 
或者是不經意的打錯字，或意外跳過部份章節。
...
B<請注意：您要使用支援 Perl 的作業系統及環境運行 Catalyst。>
Catalyst 操作上應該沒有什麼或只有一點點不同，B<但是本教學手冊是在虛擬電腦 Debian 為基礎的作業系統執行>
，您可以下載來一步步配合教學手冊進行。
B<我們強烈建議您使用虛擬電腦映像檔進行教學>來避免因為設定檔的不同打斷您的學習。
我們已經確認過教學映像檔中的範例正確執行，並很難保證您在其他作業系統可以。
...
如果您偏好直接從 CPAN 安裝這個教學手冊而不安裝教學映像檔，您可以下載範例程式 L<Task::Catalyst::Tutorial> 及所有依賴的發行模組：
...
這指令順便測試您的模組依賴可以正確安裝。
如果您在安裝時出現問題，請在 IRC 頻道 B<#catalyst> 尋求協助，或是 Catalyst 新聞群組。
...
如何編寫 Catalyst 的 CURD (新增，讀取，更新，及刪除) 操作
...
使用 SQLite 資料庫 (程式也可提供 MySQL 和 PostgreSQL 使用)。
(請注意：因為我們使用 DBIx::Class 物件關聯映射 [ORM] 層，
在我們應用程式之外會變成資料庫不明確 (database agnostic) 及很容易被 C<DBIx::Class> 支援的資料庫使用。)
...
使用 L<HTML::FormFu> 或 L<HTML::FormHandler> 自動表單處理及驗證。
...
本教學手冊以教學流程為要務。例如，程式碼中的註釋等級對於 "普通的專案" 數量及比重或許太多。
因為上下文語境，教學手冊比較偏好程式碼內的註釋而不是分段的文字討論。
也會刻意展示各種功能的多種作法 (通常，您應該盡量堅持編寫自己產品的程式碼)。
...
此外，本教學手冊嘗試最少量化控制器，模型，TT 模板，資料庫及資料表。
雖然結果有時會顯得不太自然，但是這些觀念對複雜的環境依然可行。
更完整及複雜的範例可以在下面找到 L<http://wiki.catalystframework.org/wiki/resources/catalystexamples> 和 
Catalyst 版本管理資源庫 
L<http://dev.catalyst.perl.org/repos/Catalyst/trunk/examples/>。

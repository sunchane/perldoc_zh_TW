...
Perl 中，執行什麼運算決定運算子的作用，不受運算元的型態影響。
例如 S<C<$x + $y>> 永遠是數字加法，就算如果 C<$x> 或 C<$y> 不含有數字，
也會先試著轉換為數字。
...
相比其他許多的動態程式語言，運算取決於第一個參數的型態。
也指出 Perl 的一些運算子有兩種版本，一個用在數字第二個用在字串比較。
例如 S<C<$x == $y>> 比較兩個數字是否相等，還有 S<C<$x eq $y>> 比較兩個字串。
...
operations.
也有少許例外：C<x> 可以亦是字串重複也可以是串列重複，根據左邊的運算元型態，
以及 C<&>，C<|>，C<^> 和 C<~> 亦可以是字串運算子也可以是數字字元運算。

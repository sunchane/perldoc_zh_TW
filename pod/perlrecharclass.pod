...
字元類別是表示一組字元的方式，用這組字元比對其中之一目標字元的方法。
請記住一個重點：比對一個字元類別只計算一個來源字串的字元。
(來源字串指的是正規表達是要比對的文字)
...
Perl 正規表達式有三種不同的字元類別：點號，反斜線序列，中括號包圍的格式。
請謹記，無論如何，"字元類別" 的形式能用中括號格式來代表。當然大部分的 Perl 說明文件這樣做。

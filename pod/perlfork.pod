...
=head1 SYNOPSIS
    ...
    請注意：Perl 5.8.0 發布後，fork() 模擬器被認為更臻成熟了。
    然而，還是存在少許知名的臭蟲以及跟真實的 fork() 不同之處影響著您，
    請看 "臭蟲" 及 "注意事項及限制" 於接下來的章節。
...
Perl 提供的 fork() 關鍵字呼應 Unix 系統呼叫使用相同的名稱。
大部分 Unix-like 系統平台通常會提供 fork() 系統呼叫。
Perl 的 fork() 間單地在程式中呼叫它。

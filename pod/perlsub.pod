...
跟其他程式語言一樣，Perl 提供使用者定義的副程式。
可以是放置在煮成是的任何地方，或從其他檔案匯入，透過使用 C<do>，C<require>，或 C<use> 關鍵字，
或是可以使用 C<eval> 或 匿名副程式動態的產生。
您甚至可以間接的用變數內的字面或參照呼叫函式。
...
Perl 函式呼叫模型及回傳值的概念很簡單：
所有的函式傳遞的參數是一串扁平的純量序列，然而同樣地給呼叫者的回傳值也是一串扁平的純量序列。
所有陣列或雜湊呼叫或回傳時會崩解，失去它們的識別 -- 但是您可以傳遞參照避免這現象。
傳遞參數或回傳值序列的數量可以根據您喜好決定。 (一個沒有回傳值的函式我們常稱為副程式，
但是在 Perl 觀點並不重要。)
...
所有傳遞的參數可以在 C<@_> 中出現。
(也會在詞法作用域中可見，簽章部份會介紹；請看下面 L</Signatures>。) 此外，
如果您呼叫一個函式使用兩個參數，會存入 C<$_[0]> 和 C<$_[1]> 之中。
陣列 C<@_> 是一個區域陣列，但是它們的元素會是實際純量參數的別名。
尤其，如果元素 C<$_[0]> 更新後，反映的參數也會更新 (或是產生參數無法更新的錯誤訊息)。
如果您呼叫的函式參數為不存在的陣列或雜湊，參數的元素只會在 (如果) 修正過或參照到之後才會建立。
(一些早期的 Perl 不管有沒有指派成功都會建立參數元素。)
指派一整個陣列 C<@_> 會刪除所有的參數別名，也不會更新任何參數。
X<subroutine, argument> X<argument> X<@_>

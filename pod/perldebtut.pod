...
perl programs.  
這是一個 (非常) 輕量級的介紹有關使用 Perl 除錯器，及對於現存，深入的除錯主題資訊來源給予指示。
...
day.  
This is for them.  
不是完全了解如何使用 Perl 除錯器的人，數量還真是多的嚇人，就算他們每天都在用，本文就是為了他們寫的。
...
a problem:
首先，很少東西能夠讓您的人生遇到 Perl 除錯一帆風順，除非是使用 Perl 除錯器。
作為演示，這裡有個簡單的腳本，叫作 "hello"，有些問題：

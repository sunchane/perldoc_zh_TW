...
see L<perlreftut>.
這裡是有關參照全部觀點的完整手冊。
對於簡短，實務功能的教學簡介，請看 L<perlreftut>。
...
Perl 發行第 5 版以前表現複雜資料是很困難的一件事，因為所有的參照必須是符號--
甚至不使用符號登錄表就不容易參照到變數。
現在 Perl 不只更容易使用變數的符號參照，更任您可以用 "硬" 參照到任何資料或程式碼。
任何的純量變數持有硬參照。因為陣列和雜湊包含純量，您可以簡單的建立陣列中的陣列，
陣列中的雜湊，雜湊中的陣列，陣列中的雜湊中的函式，以此類推。

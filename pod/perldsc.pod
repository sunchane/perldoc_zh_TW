...
Perl 讓您擁有複雜的資料結構。 您可以瞬間寫出下面的東西，三維的的陣列！
...
您要如何印出？ 為什麼不能只用 C<print @AoA>? 您要如何排序它？
您要如何傳遞到函數式或透過函式回傳這個？ 它是物件嗎？ 您能夠存入磁碟又讀取嗎？
您如何存取整個矩陣的列或行？ 全部的值必須是數字嗎？
...
如您所見，很容易就讓您困惑了。 這玩意的小部份罪過歸咎於參照的實作品基礎，
範例的說明文件缺乏對新手來說主題太大。
...
這份說明文件為了詳細但又不失容易理解，在您開發時所用到的各資料結構。
也可以當作範例食譜。 如此一來，當您需要建立其中一個複雜的資料結構時，
您可以抓，偷，盜並且在您的程式貼上。
...
最重要的事您必須了解的是 Perl 全部的資料結構包括多維陣列儘管看不太出來，
Perl C<@陣列> 及 C<%雜湊> 內部都是一維的。 您只能擁有純量 (意思是字串，數字，或是參照)。
它不能夠直接存放其他陣列或雜湊，只能夠存放其他陣列或雜湊的 I<參照>。
...
您不能夠把參照當作真的陣列或雜湊的方式使用。 對於 C 或 C++ 的程式設計師也不習慣把陣列跟指針當成一樣的，這讓人困惑。
如果要這樣，就把它想成資料結構跟資料結構的指針的關係。
...
您可以閱讀 (應該) 更多有關參照的說明 L<perlref>。
聰明地說，參照比起指針應該更像是指向什麼東西。 (物件也是一種參照，但我們目前不需要，如果曾經有的話。)
這是指當您有個二維或多維的陣列/雜湊要查看時，真正發生的事不過是一維的物體中包含了之指向下一層的參照。
這只不過是您可以當成二維 I<使用>。 事實上跟 C 的多維陣列工作方式幾乎一樣。

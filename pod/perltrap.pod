...
L<perldelta>.
所有事情的最大的陷阱就是忘記加 C<use warnings> 或用 B<-w> 於命令列；
請看 L<warnings> 和 L<perlrun/-w>。
第二大的的陷阱是讓您所有執行的程式沒用到 C<use strict>。
第三大的陷阱是不讀 Perl 最新版本的變化列表；請看 L<perldelta>。
...
Perl 程式只執行一次，不是每次輸入執行一次。
您可以暗地使用 C<-n> 或 C<-p> 執行迴圈。

...
不管您在 Perl 如何使用 I/O 存取檔案，您一定是透過使用 Perl 稱為B<檔案代號>的方式。
檔案代號是使用外部檔案的內部名稱。 將外部檔案跟內部名稱產生關聯是 C<open> 的工作。
而 C<close> 函式的工作是斷開這個關聯。
...
為了您的方便，Perl 一開始就設置了許多特別的檔案代號。
包含 C<STDIN>，C<STDOUT>，C<STDERR>，和 C<ARGV>。
因為它們預先已經打開了，您可以馬上使用它們，不需要您自己打開還要顧慮各種麻煩。
...
如您所見上面例子，C<STDOUT> 和 C<STDERR> 是一個輸出的檔案代號，以及 C<STDIN> 
和 C<ARGV> 是一個輸入的檔案代號。它們都是大寫字母因為他們是 Perl 的保留字，
跟 C<@ARGV> 內建陣列和 C<%ENV> 內建雜湊相同。它們的外部連結設定為您的命令列程式視窗。
...
您自己會需要開啟任何其他的檔案代號。 雖然它們有很多不同之處，Perl 最常見的方法是使用 open() 函數
配合三個參數及一個回傳值。
...
C<open> 函式最複雜的地方應該是 I<MODE> 參數根據您的需要不同。

...
這是有關效能及優化技術的簡介，可以和特定的 Perl 程式一同參考。
很多 Perl 的開發人員來自於其他程式語言，並且可以適當的利用他們先前的知識，
有一些人也從 Perl 的少數建議的到益處。
如果您想要本文的一個濃縮版本，也許最好的一句話來自日本宮本武臟：
"不做無用之事"
...
或許程式設計師最常見的的錯誤，就是明明程式還沒完全上路就想要優化程式碼 - 不是個好想法。
程式如果無法順利工作跑再快也是惘然，最優先的工作應該是把程式 I<正確的> 把某件事 B<變有用>，
(還談不上確保測試套件達到全功能)，然後才來談優化的問題。
決定要優化現有工作中的程式後，有幾個簡單但是實用的步驟可以考慮，真正有用的優化程序。
...
首先，您必須先設立現有程式碼時間線的基準點，時間計算要可靠也要可重複的。
這階段您大概會想用 C<Benchmark> 或 C<Devel::NYTProf> 套件，或相似的，
或是使用 Unix 系統裡的 C<time> 工具套件，不管哪個比較適合。
請觀看本文稍長有關基準化及分析模組，及推荐閱讀的清單。

...
每一位程式設計師，當然會有他自己的格式喜好，但是這裡有一些常見的指導方針，
可以讓您的程式碼容易閱讀，理解，以及維護。
...
最重要的事就是在您所有的程式裡使用 L<strict> 和 L<wranings> 或了解為什麼不用。
您也許會需要在部份程式段落關閉透過 C<no warnings> 或 C<no strict>，關閉後後就限制了特定的警告或約束。
命令列 B<-w> 旗標和 C<$^W> 變數不應該用在這個目的，因為那是影響您在執行程式的階段而不是編寫時，
例如編寫核心或 CPAN 模組。
...
根據程式碼編排的美學，Larry 只重視一件事情就是包在大括號中的多行區塊結構，關鍵字要排列整齊。
除了這個，其他不太強調的偏好如下：

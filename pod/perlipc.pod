...
perlipc - Perl 行程間通訊 (信號, 先進先出, 導管, 安全副行程, 接口, 號誌)
...
Perl 簡單的 IPC 設施是由優良又古老的 Unix 信號，具名導管，導管開啟，柏克萊接口常式，
以及 SysV IPC 呼叫。每一個的使用場合都有些微差異。
...
Perl 使用一個簡單的信號處理模型：$SIG 雜湊包含名稱或使用者安裝的信號處理常式參照。
這些常式呼叫包含被觸發的信號做為參數。 信號可能產生自明確的意圖如鍵盤輸入特定的按鍵 Ctrol-C 或 Contrl-Z，
從其他行程傳來的信號，或自動從核心觸發特殊的事件，像是子行程結束，您的行程用完了堆疊空間，
或發生檔案大小限制。


For example, to trap an interrupt signal, set up a handler like this:
舉例來說，內部信號捕捉設定，設置一個處理常式如下：

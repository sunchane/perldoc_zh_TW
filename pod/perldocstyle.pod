...
perldocstyle - 編寫 Perl 官方說明文件風格指引。
...
好幾十個的說明文件檔案命名開頭為 "C<perl>"，例如 C<perlobj>，C<perlre>，以及 C<perlintro>。
(沒錯，C<perl>。)
...
Perl 發行版包含的所有模組說明文件 (如同下列 L<C<perlmodlib>|perlmodlib>)。

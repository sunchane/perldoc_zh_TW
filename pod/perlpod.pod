...
=for comment
本文使用 POD 格式。閱讀本文，須使用 POD 格式閱讀器，像是 "perldoc perlpod"。
...
您的說明文件段落中大部分是個文字區塊，像是這一段。
您可以簡單的編寫您的文字而不使用標記語言，段落前後使用空白行。
當格式化時，文件會經歷最少的格式化，像是重新換行，平均分配單字間空白，
還可能對齊文稿。
...
section, below.
您可以使用在一般的段落使用格式化編碼，B<bold>,
I<italic>, C<code-style>, L<hyperlinks|perlfaq>, 以及其他。
這些編碼於下面 "L<Formatting Codes|/"Formatting Codes">" 章節解釋。
s
...
逐字段落同常使用在表現程式碼區塊，或有些文字不希望使用任何特殊格式化解析，
以及不能夠換行。
...
nothing else.
逐字段落的區別為第一個字元之前加上空白或 TAB。
(通常，這個段落的每一行前面都有空白或 TAB。)
它們應該會忠實的呈現，TAB 會被假設使用 8 行的邊界。
這段落不使用特殊格式化編碼，所以您不能使用任何像斜體的功能。 \ 就是說 \，啥也不會發生。
...
指令段落為文件整個詞塊做特殊處理，通常是使用在標題或部份列表。
...
are
所有的指令段落以 "=" 開頭，緊接著標識符，後面接著任意您所請求的指令。
目前可使用的指令為

...
這一章的函式提供您在表達式的用法。
他們漸漸變成兩大主題：串列操作符和具名一維操作符。
差別在它們之間的優先級，以及其接續的逗號。 (請看優先級表在 L<perlop>。)
串列運算符處理一個以上的參數。 一維操作符只能不能處理一個以上的參數。
所以，逗號為一維運算符的分隔終止，但是串列運算符很少使用逗號分隔參數。
一維運算符通常在純量語境下使用參數，串列運算符可能在純量或串列語境下使用參數。
如果同時使用，純量參數放在開頭接者串列參數，並且只能夠放進一個串列參數。 例如，
L<C<splice>|/splice ARRAY,OFFSET,LENGTH,LIST> 有 3 個純量參數隨後一個串列參數，
而 L<C<gethostbyname>|/gethostbyname NAME> 有 4 個純量參數。
...
以下的語法說明，串列運算符使用串列為參數時表示成 LIST (並且為這個串列提供串列語境)。

...
Perl 變數名稱可以有幾個格式。
通常，它們必須以字母或下劃線開頭，任意長度 (內部限制為 251 個字元) 以及可以包含字母，數字，下劃線，或特殊序列 C<::> 或 C<'>。
這種情況，在 C<::> C<'> 之前的部份為 I<套件量詞>；請看 L<perlmod>。
非 ASCII 編碼的萬國碼字母，除非您使用 S<C<"use utf8">> 編譯命令生效，否則不能代表是字母，
以及稍微複雜的規則需要遵守；請看 L<perldata/Identifier parsing> 詳細介紹。
...
match.
Perl 變數名稱也可以是數字序列，一個標點符號，或是兩個符號序列：C<^> (插入符或抑揚符)
緊接著 C<[][A-Z^_?\]> 這些符號。這些名稱為 Perl 特殊用途的保留字；
舉例來說，所有的數字名稱使用在正規表達式匹配後存放的資料。
...
自從 Perl v5.6.0 之後，Perl 變數名稱也可以在字母數字前加上插入符。
這種方式必須要用這種格式 C<${^Foo}>；大括號不能夠省略。
C<${^Foo}> 表示純量變數名稱設定成 Ctrl-C<F> 緊接著兩個 C<o>。
這種變數保留在未來 Perl 的特殊用途，以 C<^_> 開頭變數 (插入符-下劃線)。
C<^_> 後面沒有接著名稱的符號在未來的 Perl 任何版本可能會有特殊的意義；
讓這名稱安全的用在程式中。 C<$^_> 本身，然而，I<本身是>保留字。
...
Perl 標識符以數字或標點符號開頭的的規則，效果影響範圍豁免於C<套件>宣告，
以及總是強制宣告在 C<main> 套件中；它們也豁免於 C<strict 'vars'> 錯誤訊息。
有些其他的名稱也豁免這些規則：
...
presently in scope.
尤其，特殊變數 C<${^_XYZ}> 總是宣告在 C<main> 套件中，不管任何 C<套件> 宣告在命名空間中。
...
以下的名稱在 Perl 有特殊的意義。
大部分的標點符號名稱都有合理的記憶表象，或類似命令列的慣用法。
不只如此，如果您想要用比較長的變數名稱，您只要說：
...
把這個編譯命令放在程式開頭。 這會讓這個套件所有的簡短名稱，可以使用長長的英文別名。
有些還可以用中等常的英文名稱，一般來說都是借鑒於 B<awk> 來的。
更多資訊，請看 L<English>。
...
繼續以前，請注意這些內建變數的順序。通常我們列出的變數是大小寫區分，依照大部分-字典序
(忽略開頭 C<{> 或 C<@_> 文字，例如 C<${^UNICODE}> 或 C<$^T>)，雖然 C<$_> 或 C<@_> 移動到比較前面。
變數如果有相同的標識符，我們以純量，陣列，雜湊，裸字順序排列。

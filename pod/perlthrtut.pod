...
本教學手冊描述了 Perl 直譯器有關執行緒的使用 (有時會用 I<ithreads>)。
這個模組，每個執行緒運行一個 Perl 直譯器，資料分享在每個執行緒間必須明確。
I<ithreads> 使用者等級的界面請用 L<threads> 類別。
...
B<請注意>：有另一個比較老式的 Perl 執行緒稱為 5.005模組使用 L<threads> 類別。
這個老式的模組有一個問題，並且已經棄用，5.10 發行版已經移除。
強烈鼓勵您將現有的 5.005 執行緒程式碼盡快遷移到新版的模組。

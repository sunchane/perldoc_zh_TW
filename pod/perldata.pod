...
Perl 有三種內建資料型別：純量，純量組成的陣列，以及純量組成的關聯式陣列，我們熟悉的 "雜湊"。
純量為一個字串 (各種大小，只限制在可用記憶體的多寡)，數字，或指向某東西的參照 (我們在 L<perlref> 討論)
。普通的陣列由純量集合組成，數字排序 0 開始。雜湊由未排序的純量集合組成，
由鍵為其關聯索引值。
...
X<identifier>
變數通常認為是名稱，或透過參照的名稱。
名稱的第一個字元告訴您指的是哪一種資料結構。
名稱其他的部份告訴您要參考到的名字。
名字通常為單一 I<識別>，也就是說，是一個開頭為字母或下畫線，內部包括字母，下畫線，及數字。
一些情況，他們是識別鏈，由 C<::> 分隔 (古老一點可能是 C<'>)；除了最後一段會被編譯成套件名稱，
搜尋命名空間並且找到最後一個做為識別 (詳細請看 L<perlmod/Packages>)。
更多深入的識別字討論，請看 L</Identifier 解析>。 它可以替換簡單的識別字，執行時期由表達式產生的變數參照。
下面有更多敘述及 L<perlref>。
X<identifier>
...
X<variable, built-in>
Perl 也有它自己內建的變數識別字不按這些規則的。
它們有奇怪的名稱所以通常不會和您正常的變數放在一起。
正規表達式比對分組小括號內的部份會把變數存在只有 C<$> 跟著數字的識別字中 (請看 L<perlop> 和 L<perlre>)。
除此之外，很多 Perl 在窗子內部運作的特殊變數會用一些標點符號組成。
這些說明手冊在 L<perlvar>。
X<variable, built-in>
...
X<scalar>
純量變數總是和 '$' 一起命名，就算它是陣列或雜湊裏面其中一個純量。
'$' 字符語法上類似英文的 "the" 代表給人認為是單個值。
...
X<array>
整個陣列 (以及從陣列或雜湊分割來的陣列) 會被標上 '@'，
用英文來說比較像 "這些" 或 "那些"，代表給人認為是多個變數。
...
另外，副程式開頭用 '&' 命名，雖然是可選的但是可以消除歧異，就像英文 "do" 這個贅詞。
整個符號表命名可以用 '*' 開頭，但是您真的還不需要關心 (直到如果有一天 :-)
...
X<namespace>
每一個變數有它自己的名稱空間，非變數也相同。
意思是您可以不用擔心會互相衝突，命名純量變數，陣列，或雜湊用相同名稱，
以此類推，檔案代號，目錄代號，副程式名稱，格式化名稱，或標籤。
這麼說 $foo 和 @foo 是兩個不同變數。
也就是說 C<$foo[1]> 是一部份 @foo，不是 $foo 的一部份。
可能感覺怪怪的，但是 OK 的，它本來就怪怪的。
...
X<case>
因為變數參照總是以 '$'，'@'，或 '%' 開頭，"保留字" 事實上也不是保留給變數使用的。
它們 I<是> 保留給標籤或檔案代號使用的，所以名稱開頭沒有特殊符號。
例如您最好不要用 "log" 為檔案代號。
提示：您可以說 C<open(LOG, 'logfile')> 而不是 C<open(log, 'logfile')> 檔案代號使用大寫
可以增加易讀性和保護您跟未來其他的保留字衝突。

It is possible to replace such an alphanumeric name with an expression
that returns a reference to the appropriate type.  For a description
of this, see L<perlref>.
可以用表達式為這些英文數字的變數，替換並回傳適當的變數變數型態。
討論這方面請看 L<perlref>。
...
名稱開頭如果是數字可能是個包含多個的數字。
名稱開頭不是字母，下畫線，數字或 B<^> 的特殊符號之後限制另外一個特殊符號，
例如， C<$%> 或 C<$$>。 (大部份這些符號的變數 Perl 有預先定義的用途)。
例如， C<$$> 代表目前程序的 ID。 所有這種名稱儘可能的保留給 Perl 使用。
...
rules apply:
直到 Perl 5.18，真正的識別符規則還很模糊。但是，通常來說，現在能運作的定義識別符，
在之前的 Perl 版本應該也能夠正常運作，相反地 -- 邊界情況前版本可運作，但是新版本沒定義 -- 
大概不會在新版本運作。
順帶說明，請注意接下來的例子在 Perl 原始碼中只適用裸字識別字，而不是先前識別符介紹過的符號參考，它有比較少的限制。
如果運作在 C<use utf8;> 編譯命令之下，適用以下規則：

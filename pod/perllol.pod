...
最簡單的二層資料結構在 Perl 中建立的方式為陣列中的陣列，有時候偶而稱之為串列中的串列。
它肯定是容易學習，而且這裡所有的運用對之後更華麗的資料結構也適用。
...
of the array:
陣列中的陣列不過是一個標準的老陣列 @AoA 用兩個下標符號而已，像是 C<$AoA[3][2]>。
這裡是陣列的宣告：
...
現在您應該會很小心的使用您的括號，是指小括號。
因為您在指派 @陣列，所以您需要用小括號。 如果您不想讓他成為 @AoA，
而只想要使用參照的方式，就使用下面這種方式：

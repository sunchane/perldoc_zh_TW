...
perlrebackslash - Perl 正規表達式反斜線序列及跳脫字元

=head1 DESCRIPTION

The top level documentation about Perl regular expressions
is found in L<perlre>.
這篇有關正規表達式說明文件的頂層可在 L<perlre> 找到。
...
本說明文件依順序描述所有反斜線及跳脫字元。
先解釋反斜線的角色，之後條列出所有 Perl 正規表達式特殊意義的項目 (字母順序)，
然後為每一個做描述。
...
每個順序大部分詳細的描述放在不同的文件；本說明文件的主要目的為快速參考指南，
依序描述所有的反斜線及跳脫字元的功能。

...
places:
最簡單的方時執行 Perl 程式是直接讓腳本可以執行，或是透過檔案名稱作為參數。
(互動式 Perl 環境是可行的 -- 請看 L<perldebug> 如何使用。)
開始時，Perl 用以下方式尋找您的程式位置。
...
指令行內透過 L<-e|/-e commandline> 或 L<-E|/-E commandline> 執行腳本內容。
...
檔案內容的第一行包含執行檔的檔案位置。(記住此系統必須支援 C<#!> 標記才能用這方法。
請看 L</Location of Perl>。)
...
暗中地傳入標準輸入。 對有些沒有檔名的輸入有用，您必須明確指定一個參數 "-" 給此程式。
...
第 2 和第 3 個方法，Perl 從開頭解析輸入的檔案，除非您有指定 L</-x> 開關，
然後就會解析 C<#!> 指令之後並且包含 perl 原始碼的內容。
對於想要坎入大量訊息內容的情況很有用。(同樣的情形您可以指定程式結束的地方 C<__END__> 指示詞。)
...
C<#!> 行解析時總是會檢查所有的開關選項。所以，如果您的機器如果只接受一個 C<#!> 參數，
更糟的，甚至無法認出 C<#!> 行，您還是可以讓 Perl 呼叫時強制使用選項切換，
就算 L</-x> 已經用在尋找程式的開頭。
...
因為歷史因素有些作業系統核心解釋器會悄悄的砍掉 C<#!> 行 32 字元之後的內容。
有些選項切換開關會通過，有些則不會；
您甚至會遇到 "-" 後面沒有字碼，如果您不在意。
您通常會在意選項切換是否因為 32 字元的關係被砍掉。
大部分的選項切換對於程式執行是多餘的，但是如果程式變成不完整的 "-" 選項開關的話，
Perl 會當作從標準輸入執行您的程式。 
如果包含 L<-I/-Idirectory> 的選項切換也會變成出乎意料的結果。
...
C<BEGIN{ $/ = "\0digits"; }>.
有些選項切換執行兩次時須注意，例如，結合使用 L<-l|-l[octnum]> 和 L<-0|-0[octalE<sol>hexadecimal]>
(如果系統適用無仿) 所有超過 32 字元的切換選項，或把 B<-0>I<digits> 改成腳本中的 C<BEGIN{ $/ = "\0digits"; }>。 

Parsing of the C<#!> switches starts wherever "perl" is mentioned in the line.
The sequences "-*" and "- " are specifically ignored so that you could,
if you were so inclined, say
解析 C<#!> 行選項開關從 perl 指令出現之後開始。
"-*" 及 "- " 之間特別會忽略掉，所以如果您很奇葩，例如：
...
上面的範例使用 Perl 直譯器的相對路徑，使用哪個版本根據使用者的路徑變數。
如果您要指定使用的 Perl 版本，您應該直接修改 C<#!> 行的路徑。
...
如果 C<#!> 行沒有包含 "perl" 還是 "indir" 的字眼，此程式不是使用 Perl 直譯器的名稱執行。
雖然有點怪異，但是可以幫助人不要使用 C<#!>，因為它告訴程式使用命列列的 F</usr/bin/perl>，
Perl 指派這個程式使用正確的直譯器。
...
定位好您的程式之後，Perl 編譯整個程式程內部的格式。
如果有太多的錯誤，就不會嘗試執行。
(跟典型的命令列腳本不太一樣，可能只執行到部份直到找到錯誤。)
...
如果程式碼語法上正確，就執行。
如果程式執行到最後沒遇到 exit() 或 die() 操作，即暗為程式中執行 C<exit()> 代表成功。 

...
本頁提供簡單的教學讓您理解，建立和使用 Perl 正規表達式。 其他的補充參考頁面在 L<perlre> 裡面。
正規表達式不可或缺的 C<m//>, C<s///>, C<qr//> 以及 C<split> 運算子，
而且與 L<perlop/"Regexp Quote-Like Operators"> 和 L<perlfunc/split> 內容重疊。
...
Perl 優秀的文字處理能力眾所皆知，也是最受歡迎的因素。
Perl 正規表達式扮演著其他語言不及的高效及彈性。 就算只學到簡單的應用，
都能夠任您在處理文字時驚嘆不已。
...
什麼是正規表達式？ 簡單來說，正規表達式是一個決定字串中哪部份的樣板。
這些字串來源通常是一些文本，如一行，一句，網頁，甚是是整本書，但是不是一定。
比如也可以是二元資料。 生物學家常使用 Perl 比對 DNA 序列的樣板。
...
假如我們想要用變數來決定比對的文字，C<$var> 是否包含一個字元序列 S<C<m u s h r o o m>>
(加入空白方便閱讀)。 我們可以把 Perl 寫成：
...
are looking for.
如果 C<$var> 的任何一處包含這一個字元序列的話，表達式結果為 TRUE，不然就是 FALSE。
包含在 C<'E<sol>'> 資源中的部份表示我們要搜尋的內容。
...
days;
我們稱此為 I<樣本>。 根據樣本執行搜尋字串的程序稱之為 I<比對>， 
而 C<m//> 旁邊的 C<=~> 運算符告訴 Perl 試著用樣本去比對字串。 請記住樣本也是字串，但是比較特別，
我們接著會看到。 樣本近來常被用到；
...
舉例來說在搜尋引擎輸入樣本搜尋網頁，和樣本條列出資料夾的檔案名稱， I<e.g.>，"C<ls *.txt>"
或 "C<dir *.*>"。 在 Perl，正規表達式的樣本所表達的不只有搜尋字串，也擷取字串中需要的部份，
以及執行搜尋和替換的運算。
